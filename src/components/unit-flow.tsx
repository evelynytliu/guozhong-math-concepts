...
export function UnitFlow({ unit }: { unit: Unit }) {
  ...
  function goTo(section: number) {
    const clamped = Math.min(5, Math.max(1, section));
    setCurrent(clamped);
    setReached((r) => {
      const nr = Math.max(r, clamped);
      void saveProgress({
        unitId: unit.id,
        sectionReached: nr,
        completedAt,
  ...
  return (
    <div className="flex flex-1 flex-col pb-10">
      ...
      <div className="sticky top-0 z-10 -mx-4 mt-4 border-b bg-background/90 px-4 py-4 backdrop-blur sm:-mx-6 sm:px-6">
        <ProgressStepper current={current} reached={reached} onJump={goTo} />
      </div>
...
function UnitCompletion({
  ...
  return (
    <div className="animate-fade-in rounded-xl border border-correct/40 bg-correct/5 p-6 text-center">
